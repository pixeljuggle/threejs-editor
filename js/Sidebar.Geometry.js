...
function SidebarGeometry(editor) {
  ...
  const geometryUUIDRenew = new UIButton(strings.getKey("sidebar/geometry/new"))
    .setMarginLeft("7px")
    .onClick(function () {
      geometryUUID.setValue(THREE.MathUtils.generateUUID());

      editor.execute(
        new SetGeometryValueCommand(editor, editor.selected, "uuid", geometryUUID.getValue())
      );
    });
  ...
  const geometryName = new UIInput()
    .setWidth("150px")
    .setFontSize("12px")
    .onChange(function () {
      editor.execute(
        new SetGeometryValueCommand(editor, editor.selected, "name", geometryName.getValue())
      );
    });
